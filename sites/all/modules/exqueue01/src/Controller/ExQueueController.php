...
class ExQueueController extends ControllerBase {
  ...
  public function deleteTheQueue() {
    //$a = $this->queueFactory->get('exqueue_import')->deleteQueue(); //print_r($a); die();
    $delete_queue = $this->deleteQueues();
    if ($delete_queue > 0) {
      return [
        '#type' => 'markup',
        '#markup' => $this->t($delete_queue.' has been deleted'),
      ];
    } else {
      return [
        '#type' => 'markup',
  ...
  public function deleteQueues() {
    $nids = \Drupal::entityQuery('node')
      ...
      ->sort('created', 'ASC')
      ->execute(); //echo '<pre>'; print_r($nids); die();

    $results = array();
    foreach ($nids as $nid) {
      $node = Node::load($nid); //echo '<pre>'; print_r($node); die();  
      $results[] = $node->delete();
    }
    ...
    //   return [
    //     '#type' => 'markup',
    //     '#markup' => $this->t(count($result . 'has been deleted')),
    //   ];
    // }
  }
  ...
  public function getData() {
    ...
    $queue = $this->queueFactory->get('cron_import');
